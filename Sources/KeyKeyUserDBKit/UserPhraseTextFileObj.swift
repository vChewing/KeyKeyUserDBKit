  ...
  public final class UserPhraseTextFileObj: Sendable, UserPhraseDataSource {
    ...
    private static func decryptPage(
      ...
      for blockIdx in 0 ..< (dataAreaSize / 16) {
        ...
        #else
          var ctx = AES_KEY()
          // Use CCryptoBoringSSL-prefixed functions on non-Darwin platforms
          let setResult = key.withUnsafeBufferPointer { keyPtr in
            CCryptoBoringSSL_AES_set_encrypt_key(keyPtr.baseAddress, 128, &ctx)
          }
          guard setResult == 0 else {
            throw TextFileError.decryptionFailed(
              message: "AES_set_encrypt_key failed with status: \(setResult)"
            )
          }
          counterBytes.withUnsafeBytes { counterPtr in
            keystream.withUnsafeMutableBytes { keystreamPtr in
              CCryptoBoringSSL_AES_encrypt(
                counterPtr.baseAddress?.assumingMemoryBound(to: UInt8.self),
                keystreamPtr.baseAddress?.assumingMemoryBound(to: UInt8.self),
                &ctx
              )
            }
          }
