...
extension KeyKeyUserDBKit {
  ...
  public struct SEEDecryptor: Sendable {
    ...
    private func aesECBEncrypt(block: [UInt8]) throws -> [UInt8] {
      #if canImport(CommonCrypto)
        return try aesECBEncryptCommonCrypto(block: block)
      #else
        return try aesECBEncryptSwiftCrypto(block: block)
      #endif
    }
    ...
    #else
      /// 使用 BoringSSL 進行 AES-ECB 加密
      private func aesECBEncryptSwiftCrypto(block: [UInt8]) throws -> [UInt8] {
        var aesKey = AES_KEY()
        let result = key.withUnsafeBufferPointer { keyPtr in
          CCryptoBoringSSL_AES_set_encrypt_key(keyPtr.baseAddress, 128, &aesKey)
        }
        guard result == 0 else {
          throw DecryptionError.cryptoError(status: Int32(result))
        }

        var outBuffer = [UInt8](repeating: 0, count: 16)
        block.withUnsafeBufferPointer { inPtr in
          outBuffer.withUnsafeMutableBufferPointer { outPtr in
            CCryptoBoringSSL_AES_encrypt(inPtr.baseAddress, outPtr.baseAddress, &aesKey)
          }
        }
        return outBuffer
      }
